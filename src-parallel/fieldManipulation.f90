...
!...Local
    ...
    do i=1,npro
      ...
      call presFaceDivInner(ijp, ijn, xf(iface), yf(iface), zf(iface), arx(iface), ary(iface), arz(iface), fpro(i), &
                            p, dPdxi, dfxe, dfye, dfze)
...
!...Local
    ...
    do i=1,npro
      ...
      call faceDivInner(ijp, ijn, xf(iface), yf(iface), zf(iface), arx(iface), ary(iface), arz(iface), fpro(i), &
                        u, dUdxi, dfxe)
...
!
!  This routine calculates the contribution of a boundary cell face to 
!  explicit Divergence of pressure field.
!
!***********************************************************************
!
...
!...Local
    ...
    ! Faces on processor boundary
    do i=1,npro
      if = iProcFacesStart + i
      ijp = owner( if )
      ijn = iProcStart + i
      call faceInterpolateCdsCorr( ijp, ijn, xf(if), yf(if), zf(if), fpro(i), u, dUdxi, ui(if) )  
    end do
...
!...Local
    ...
    do i=1,npro
      ...
      call faceInterpolateCdsCorr( ijp, ijn, xf(if), yf(if), zf(if), fpro(i), u, dUdxi, ui )
