...
!
  ...
  if(lcal(ien)) then
    ...
    allocate( dTdxi( 3,numCells + npro ), stat=ierr) 
  ...
  ! Mass flows trough inlet, outlet, and O- C- grid cuts, processor boundaries
  ...
  allocate( fmpro( npro ), stat=ierr) 
          if(ierr /= 0)write(*,*)"allocation error: fmpro"
