...
!
 implicit none
 integer, intent(in) :: NTYPE
 integer, parameter :: nofael_NTYPE1 = 2 ! no. of nodes in element-here Line
 integer, parameter :: nofael_NTYPE2 = 3 ! no. of nodes in element-here Tri
 integer, parameter :: nofael_NTYPE3 = 4 ! no. of nodes in element-here Quad
 integer, parameter :: nofael_NTYPE4 = 4 ! no. of nodes in element-here Tet
 integer, parameter :: nofael_NTYPE5 = 6 ! no. of nodes in element-here Hex
 integer, parameter :: nofael_NTYPE6 = 5 ! no. of nodes in element-here Prism
 integer, parameter :: nofael_NTYPE7 = 5 ! no. of nodes in element-here Pyramid
...
!
 ...
 if(NTYPE.eq.1) then
   ntypeCappuccino = cappuccinoNTYPE1 
 elseif(NTYPE.eq.2) then
   ntypeCappuccino = cappuccinoNTYPE2 
 elseif(NTYPE.eq.3) then
   ntypeCappuccino = cappuccinoNTYPE3 
 elseif(NTYPE.eq.4 .or. NTYPE.eq.11 .or. NTYPE.eq.29) then
   ntypeCappuccino = cappuccinoNTYPE4 
 elseif(NTYPE.eq.5 .or. NTYPE.eq.17 .or. NTYPE.eq.12) then
   ntypeCappuccino = cappuccinoNTYPE5 
 elseif(NTYPE.eq.6 .or. NTYPE.eq.18 .or. NTYPE.eq.13) then
   ntypeCappuccino = cappuccinoNTYPE6 
 elseif(NTYPE.eq.7 .or. NTYPE.eq.19 .or. NTYPE.eq.14) then
   ntypeCappuccino = cappuccinoNTYPE7 
 else
   ntypeCappuccino = 0
 endif
...
!
 ...
 select case (iface)
    case (1)
       vertices(:) = (/ NODE(2), NODE(6), NODE(5), NODE(1) /)
    case (2)
       vertices(:) = (/ NODE(8), NODE(7), NODE(3), NODE(4) /)
    case (3)
       vertices(:) = (/ NODE(1), NODE(5), NODE(8), NODE(4) /) 
    case (4)
       vertices(:) = (/ NODE(7), NODE(6), NODE(2), NODE(3) /)
    case (5)
...
!
 ...
 subroutine addTaggedBFaceToTotalNumber( tag, PhysicalNameTag, PhysicalNameNum, len )
 !
 ! Boundary faces have physical name tags and physical integer tags
 ! here you add one to total number of tagged boundary faces where 
 ! PhysicalNameTag matches given integer tag.
 !
 implicit none
 integer, intent(in) :: tag,len
 integer, dimension(len), intent(in) :: PhysicalNameTag
 integer, dimension(len), intent(inout) :: PhysicalNameNum
 ! Locals
 integer :: i

 do i=1,len
   if( PhysicalNameTag(i).eq.tag )  PhysicalNameNum(i)= PhysicalNameNum(i)+1
 enddo
 ...
 subroutine find_pos(target_value, intArray, length, indx)
!
! Find first appearance of an element in an array
!
 implicit none
 integer, intent(in) :: target_value,length
...
! Locals
 ...
 subroutine find_offset_positions(ia,nnz,ioffset,nel)
! Call after sorting IA, and JA
! Sorting should be with raising index numbers of IA
 implicit none
 integer, intent(in) :: nnz,nel
 integer, dimension(nnz), intent(in) :: ia
 integer, dimension(nel), intent(inout) :: ioffset
! Locals
 integer :: indx
 integer :: id
